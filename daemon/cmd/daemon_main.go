...
func startDaemon(d *Daemon, restoredEndpoints *endpointRestoreState, cleaner *daemonCleanup, params daemonParams) {
	...
	go func() {
		...
		if len(d.restoredCIDRs) > 0 {
			...
			if err != nil {
				log.WithError(err).Warning("Failed to retain Identity references after " + option.IdentityRestoreGracePeriod)
			}
			wg.Wait()
			if len(newIdentities) > 0 {
				d.ipcache.UpsertGeneratedIdentities(newIdentities, toRelease)
				log.WithFields(logrus.Fields{
					logfields.Count: len(newIdentities),
				}).Info("Completed FQDN rule identity recalculation")
			}
			if len(toRelease) > 0 {
				log.WithFields(logrus.Fields{
					logfields.Count: len(toRelease),
